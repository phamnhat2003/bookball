...
exports.updateFieldType = async (req, res) => {
    try {
        ...
        // Kiểm tra xem loại sân bóng mới không trùng với các loại sân bóng khác
        if (type !== undefined) {
            const [existingFieldTypes] = await db.execute(
                'SELECT id FROM field_types WHERE type = ? AND id != ?',
                [type, id]
            );
            if (existingFieldTypes.length > 0) {
                return res.status(200).json({ message: 'Loại sân bóng đã tồn tại' });
        ...
        // Cập nhật cả type và status
        await db.execute(
            'UPDATE field_types SET type = ?, status = ? WHERE id = ?',
            [type, status, id]
        );
