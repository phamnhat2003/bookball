...
exports.updateOrder = async (req, res) => {
    try {
        ...
        // Kiểm tra booking tồn tại hay không
        const [bookingRows] = await db.execute('SELECT * FROM bookings WHERE id = ?', [bookingId]);
        if (bookingRows.length === 0) {
            return res.status(404).json({ message: 'Booking not found' });
        }

        // Cập nhật description và status
        await db.execute(
            'UPDATE bookings SET description = ?, status = ? WHERE id = ?',
            [description, status, bookingId]
        );
